...
async function createSchema(): Promise<void> {
  ...
  `);
  // Clean up removed Stebby integration tables
  await pool.query(`DROP TABLE IF EXISTS stebby_tickets`);
  await pool.query(`DROP TABLE IF EXISTS stebby_persons`);
  await pool.query(`DROP TABLE IF EXISTS stebby_config`);
}
